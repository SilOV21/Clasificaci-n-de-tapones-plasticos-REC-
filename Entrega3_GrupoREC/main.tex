...
% Hace que TikZ respete los caracteres activos de babel-spanish (. , < >); sin esta
% librería las coordenadas y las puntas de flecha rompen todos los diagramas.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Todas las reuniones de coordinación han sido realizadas y documentadas.
  \end{itemize}
\end{itemize}
\clearpage
\mywphead{WP-200 -- Diseño de la interfaz y arquitectura del sistema}
...
El diagrama de Gantt definitivo refleja la planificación temporal real ejecutada a lo largo del proyecto.
Las únicas desviaciones respecto al cronograma de la Entrega~2 son el desplazamiento de las tareas 304,
306 y 307 del WP-300 hasta el 12/04/2026 (justificado en la Entrega~2) y la incorporación de las tareas
del WP-200 a partir del 08/04/2026. El resto del cronograma se ha ejecutado sin modificaciones.

\vspace{8pt}
\begin{center}
  \setlength{\fboxrule}{0.6pt}
  \fbox{\parbox[c][5cm][c]{\dimexpr\textwidth-2\fboxsep-2\fboxrule}{%
    \centering\color{DarkGray}\large
    %% Sustituir por: \includegraphics[width=\textwidth]{gantt_definitivo.png}
    [ Insertar diagrama de Gantt definitivo aquí ]}}
\end{center}
\begin{center}
  {\small\color{DarkGray}\textit{Figura 1. Diagrama de Gantt definitivo del Proyecto REC.}}
\end{center}
...
\begin{itemize}
  ...
  \item \textbf{Clasificación} (\code{/tapones/caja\_asignada}, \code{std\_msgs/Int32}): codificación
        1--6 = \{rojo, azul, verde, amarillo, negro, blanco\}. El controlador acepta también cadenas de
...
% =====================================================================
\subsubsection{Infraestructura (\texorpdfstring{\code{lra\_vision}}{lra\_vision})}
...
y los intrínsecos como \code{sensor\_msgs/CameraInfo}
(Tabla~\ref{tab:camera-params}). La conversión a matrices OpenCV se realiza con
\code{cv\_bridge}.

\begin{table}[H]
\centering
\caption{Parámetros de adquisición (\code{camera\_manager.launch.py}).}
\label{tab:camera-params}
\small
\begin{tabularx}{\linewidth}{@{}l l X@{}}
\toprule
\textbf{Parámetro} & \textbf{Valor} & \textbf{Significado}\\
\midrule
\code{video\_device} & \code{/dev/video2} (auto) & Dispositivo V4L2\\
\code{image\_size} & \([640,480]\) & Resolución\\
\code{time\_per\_frame} & \([1,60]\) & Periodo $\Rightarrow$ 60\,FPS\\
\code{pixel\_format} & \code{rgb8} & Formato de píxel\\
\code{camera\_frame\_id} & \code{camera\_optical\_frame} & Marco óptico\\
\code{camera\_info\_url} & \code{file://.../camera\_info.yaml} & Intrínsecos\\
\bottomrule
\end{tabularx}
\end{table}
...
parada de emergencia. Está implementada en \textbf{PyQt5} e integra el bucle de
eventos de Qt con el cliente \code{rclpy} en un único hilo, evitando la
complejidad de un \textit{executor} multihilo de ROS\,2.

\subsubsection{Arquitectura}

La HMI se estructura en una ventana principal (\code{MainWindow}) y tres
componentes desacoplados (Figura~\ref{fig:hmi-arq}):
\begin{itemize}
\item \code{RosBridge}: \code{QObject} que posee el nodo \code{rclpy} y reemite
cada tópico como una señal Qt. Se suscribe a la telemetría y a las cámaras con QoS
\textit{best-effort} (perfil de sensor) y al tópico \textit{latched}
\code{/clasificador/num\_cajas} (RELIABLE + TRANSIENT\_LOCAL); convierte los
ángulos articulares de radianes a grados y publica \code{/vision\_enable}.
\item \code{ProcessManager}: lanza/para/reinicia cada subsistema como
subproceso en su propio grupo de procesos.
\item \code{ConnectionMonitor}: \code{QThread} que hace \emph{ping} ICMP periódico
(cada 2\,s) al robot y reporta su accesibilidad y el RTT.
\end{itemize}
...
\paragraph{Concurrencia.} La interfaz opera en un único hilo Qt. Un \code{QTimer}
dispara cada 50\,ms una llamada a \code{rclpy.spin\_once(timeout\_sec=0.0)}, de
modo que todos los \textit{callbacks} ROS se ejecutan en el hilo de la GUI y
actualizan los \textit{widgets} sin riesgo de condiciones de carrera. Los únicos
hilos concurrentes son el \code{ConnectionMonitor} y un hilo lector de
\code{stdout} por cada subproceso activo; ambos se comunican con la interfaz
exclusivamente mediante señales Qt encoladas, que Qt entrega de forma segura en el
hilo principal.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{Screenshot from 2026-05-29 00-30-25.png}
...
EMERGENCIA} y \textbf{REHABILITAR}) y cinco pestañas (Panel, Cámara, Registros,
Ajustes y Acerca de). La pestaña Panel reúne, a la izquierda, el lanzador y los
contadores y, a la derecha, el estado del robot y la vista de cámara en vivo.

\subsubsection{Gestión de procesos}

\code{ProcessManager} lanza cada subsistema (Tabla~\ref{tab:subsistemas}) con
\code{subprocess.Popen} y \code{preexec\_fn=os.setsid}, creando un grupo de
procesos propio. Al detenerlo envía \code{SIGTERM} a todo el grupo y, si no
termina en 5\,s, escala a \code{SIGKILL}; la parada de emergencia emplea
\code{SIGKILL} directo. La salida estándar de cada grupo se vuelca en un búfer
circular de 2000 líneas que alimenta las pestañas de registro. Cada subsistema
sigue la máquina de estados de la Figura~\ref{fig:fsm}.
...
La transición de STARTING a RUNNING es temporal: transcurrido el retardo de
arranque propio del grupo (\code{startup\_delay\_s}), si el proceso sigue vivo se
promueve a RUNNING, sin sonda de salud específica; una salida con código distinto
de cero lo lleva a CRASHED. El arranque global (\textbf{Iniciar todo}) lanza los
grupos de forma secuencial respetando el retardo de cada uno.

\begin{table}[H]
\centering
\caption{Subsistemas gestionados por la HMI (en orden de arranque).}
\label{tab:subsistemas}
\small
\setlength{\tabcolsep}{4.5pt}
\begin{tabular}{l l l c}
\toprule
\textbf{Clave} & \textbf{Sección} & \textbf{Nodo / lanzamiento} & \textbf{Retardo (s)}\\
\midrule
driver       & Hardware & \code{ur\_robot\_driver}\,\textperiodcentered\,\code{ur\_control.launch.py}    & 2.0\\
tf           & Hardware & \code{tf2\_ros}\,\textperiodcentered\,\code{static\_transform\_publisher}      & 0.5\\
moveit       & Control  & \code{ur\_moveit\_config}\,\textperiodcentered\,\code{ur\_moveit.launch.py}    & 4.0\\
camera       & Visión   & \code{v4l2\_camera}\,\textperiodcentered\,\code{v4l2\_camera\_node}            & 1.0\\
camera\_urdf & Visión   & \code{lra\_vision}\,\textperiodcentered\,\code{upload\_urdf.launch.py}         & 0.5\\
calibrator   & Visión   & \code{rec\_vision}\,\textperiodcentered\,\code{color\_calibrator\_node}        & 1.0\\
pick\_sort   & Control  & \code{ur3\_vision\_control}\,\textperiodcentered\,\code{ur3\_pick\_sort}       & 3.0\\
detector     & Visión   & \code{rec\_vision}\,\textperiodcentered\,\code{detector\_tapones}              & 2.0\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
\item \textbf{Telemetría.} La HMI incrementa el contador de la caja recibida,
muestra el total y calcula la tasa de clasificación en vivo (tapones/min desde el
inicio de sesión); permite exportar un informe JSON con la duración
(\code{duration\_seconds}), el total (\code{total\_caps}) y el conteo por caja
(\code{per\_box}). Presenta los ángulos articulares —que marca como obsoletos si
no llegan datos durante más de 2\,s— y el color del último tapón.
\item \textbf{Seguridad.} \textbf{PARADA DE EMERGENCIA} publica
\code{/vision\_enable}=False y mata todos los grupos de procesos con
\code{SIGKILL}; \textbf{REHABILITAR} vuelve a publicar \code{/vision\_enable}=True
pero \emph{no} relanza los subsistemas, que el operador debe rearrancar desde el
lanzador. \code{/vision\_enable} es el mismo tópico \textit{latched} del contrato
visión--control (Tabla~\ref{tab:contrato}), por lo que el detector queda inhibido
hasta la rehabilitación. El \code{ConnectionMonitor} señala además la pérdida de
conexión con el robot.
\item \textbf{Configuración.} La pestaña Ajustes edita los
parámetros de cada subsistema (cámara, calibrador, detector, pick \& sort) y los
persiste en \code{\textasciitilde/.lra\_hmi.yaml}, fusionado en profundidad sobre
los valores por defecto del paquete (\code{default\_config.yaml}); al cambiar
parámetros que afectan a un subsistema en ejecución, avisa de que debe reiniciarse.
\item \textbf{Modo simulación.} Con la variable de entorno \code{LRA\_HMI\_SIM}
activa, la HMI sustituye los grupos reales por nodos simulados de
\code{lra\_hmi\_sim} y apunta el LED de conexión a \code{127.0.0.1}, lo que permite
probar la interfaz sin robot ni cámara físicos.
\end{itemize}
...
\textit{[ Pendiente: incluir planos mecánicos del efector / soporte de cámara. ]}

\subsection{Planos electrónicos}
\textit{[ Pendiente: incluir esquemas electrónicos / conexionado de E/S. ]}

\section{Lista de materiales}
\textit{[ Pendiente: incluir la lista de materiales (BOM) del sistema. ]}
...
\textit{[ Pendiente: incluir la especificación de requisitos funcionales y técnicos. ]}
