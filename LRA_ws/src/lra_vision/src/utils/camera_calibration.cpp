// =============================================================================
// LRA Vision Package - Camera Calibration Implementation
...
// =============================================================================
// CalibrationConfig Implementation
// =============================================================================
...
// =============================================================================
// CalibrationResult Implementation
// =============================================================================
...
void CalibrationResult::save_to_yaml(const std::string& filepath) const
{
  ...
  out << YAML::Key << "data" << YAML::Flow << YAML::BeginSeq;
  for (int i = 0; i < 9; ++i) {
    out << (i % 4 == 0 ? 1.0 : 0.0); // Identity matrix
  }
  out << YAML::EndSeq << YAML::EndMap;
  ...
  out << YAML::Key << "data" << YAML::Flow << YAML::BeginSeq;
  // First 3x3 is camera matrix, last column is zeros
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 3; ++j) {
      out << camera_matrix.at<double>(i, j);
    }
...
std::string CalibrationResult::to_camera_info_yaml() const
{
  ...
  out << YAML::Key << "cols" << YAML::Value << 1;
  out << YAML::Key << "data" << YAML::Flow << YAML::BeginSeq;
  // Ensure we have 5 coefficients (pad if necessary)
  for (int i = 0; i < 5; ++i) {
...
bool CameraCalibrator::add_image(const cv::Mat& image, bool visualize)
{
  ...
  // Initialize image size from first image
...
bool CameraCalibrator::add_image_from_file(const std::string& filepath, bool visualize)
{
  ...
  if (add_image(image, visualize)) {
    calibration_data_.image_paths.push_back(filepath);
    return true;
  }
...
CalibrationResult CameraCalibrator::calibrate()
{
  ...
  // Set initial focal length estimate
  ...
  undistort_maps_computed_ = false;
  
  return result;
}

cv::Mat CameraCalibrator::undistort_image(const cv::Mat& image) const
{
  ...
  cv::Mat undistorted;
  cv::undistort(image, undistorted, calibration_result_.camera_matrix, calibration_result_.dist_coeffs);
  return undistorted;
}

void CameraCalibrator::get_undistortion_maps(cv::Mat& map1, cv::Mat& map2) const
{
  if (!calibration_result_.success) {
    return;
  }
  
  if (!undistort_maps_computed_) {
    cv::Mat optimal_matrix = cv::getOptimalNewCameraMatrix(
      calibration_result_.camera_matrix,
      calibration_result_.dist_coeffs,
      calibration_data_.image_size,
      1.0,
      calibration_data_.image_size
    );
    ...
    undistort_maps_computed_ = true;
  }
  
  map1 = undistort_map1_.clone();
  map2 = undistort_map2_.clone();
}
...
std::optional<CalibrationResult> CameraCalibrator::get_result() const
{
  if (calibration_result_.success) {
...
bool CameraCalibrator::load_calibration(const std::string& filepath)
{
  ...
  if (calibration_result_.success) {
    calibration_data_.image_size = cv::Size(
      calibration_result_.image_width,
      calibration_result_.image_height
    );
    undistort_maps_computed_ = false;
    return true;
  }
...
)
{
  ...
  if (found && !corners.empty()) {
    cv::Size pattern_size(9, 6); // Default, should be configurable
    cv::drawChessboardCorners(display, pattern_size, corners, found);
  }
