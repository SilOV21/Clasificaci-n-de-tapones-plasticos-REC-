// =============================================================================
// LRA Vision Package - Camera Detector Implementation
...
std::vector<std::string> CameraDetector::scan_video_devices()
{
  ...
  while ((entry = readdir(dir)) != nullptr) {
    ...
    // Check if it starts with "video" AND has characters after it
    if (name.substr(0, 5) == "video" && name.length() > 5) {
      // Ensure the rest of the string is purely digits (ignores things like video-metadata)
  ...
  std::sort(devices.begin(), devices.end(), [](const std::string& a, const std::string& b) {
    // Changed 11 to 10! Because we validated with isdigit above, stoi is now 100% safe.
