...
class UR3PickSort(Node):
    ...
    def __init__(self):
        ...
        self.front_bin_waypoint = self.normalize_joint_vector([
             math.radians(-40),
             math.radians(-95),
             math.radians(70),
             math.radians(185),
        ...
        self.bin_drop_xyz = {
            'rojo':     (0.00, -0.46, 0.15),
            'azul':     (0.08, -0.46, 0.15),
            'verde':    (0.18, -0.46, 0.15),
            'amarillo': (0.28, -0.46, 0.15),
        }
    ...
    def execute_pick_and_place(self, target, color):
        ...
        if self.use_bins:
            ...
            # 5.1) Ir primero a un waypoint frontal seguro
            ok = self.move_to_joint_waypoint_blocking(
                self.front_bin_waypoint,
                move_time_sec=4,
                timeout_sec=8.0
            )
            if not ok:
                self.get_logger().warn('Falló front_bin_waypoint antes de ir a la caja.')
                self.return_home_and_wait(timeout_sec=7.0)
                self.busy = False
                self.enable_vision(True)
                return

            # 5.2) Ir al approach de la caja
            ok = self.move_to_joint_waypoint_blocking(
                self.bin_approach_joints[color],
                move_time_sec=6,
                timeout_sec=10.0
            ...
            # 5.5) Volver al approach si hace falta
            ...
            # 5.6) Pasar otra vez por el waypoint frontal
            ok = self.move_to_joint_waypoint_blocking(
                self.front_bin_waypoint,
                move_time_sec=4,
                timeout_sec=8.0
            ...
            if not ok:
                self.get_logger().warn('Falló front_bin_waypoint al regresar de la caja.')
                self.return_home_and_wait(timeout_sec=7.0)
                self.busy = False
                self.enable_vision(True)
                return
