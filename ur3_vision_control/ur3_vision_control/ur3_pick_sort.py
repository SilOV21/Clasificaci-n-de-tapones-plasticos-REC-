...
class UR3PickSort(Node):
    ...
    def __init__(self):
        ...
        # Punto previo común seguro antes de entrar a la zona de cajas
        self.left_pre_bin_waypoint = self.normalize_joint_vector([
            deg(-220),
            deg(-85),
            deg(-105),
            deg(-80),
        ...
        amarillo_drop = [
            amarillo_point[0],
            amarillo_point[1] - deg(8),
            amarillo_point[2] + deg(8),
            amarillo_point[3],
            amarillo_point[4],
            amarillo_point[5],
        ]
    ...
    def execute_pick_and_place(self, target, color):
        ...
        if self.use_bins:
            ...
            ok = self.move_to_joint_waypoint_blocking(
                self.left_pre_bin_waypoint,
                move_time_sec=6,
                timeout_sec=18.0
            )
            if not ok:
                self.get_logger().warn('Falló left_pre_bin_waypoint.')
